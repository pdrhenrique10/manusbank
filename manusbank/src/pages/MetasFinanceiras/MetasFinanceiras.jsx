...
function MetasFinanceiras() {
  ...
  useEffect(() => {
    async function carregarMetas() {
      try {
        ...
        setErro("Não foi possível carregar as metas financeiras.");
      } finally {
        setCarregando(false);
      }
    }
  ...
  const handleAdicionarMeta = async (e) => {
    ...
    if (!novaMeta.titulo || !novaMeta.valorAlvo || !novaMeta.dataMeta) {
      alert("Preencha título, valor objetivo e data da meta!");
      return;
    }
    ...
    try {
      ...
      const body = {
        ...
        dataMeta: novaMeta.dataMeta, // "YYYY-MM-DD"
  ...
  // Aportar dinheiro em uma meta -> PATCH /api/metas/:id/aportar
  const handleAportar = async (metaId) => {
    if (!valorAporte) {
      alert("Digite um valor para aportar.");
      return;
    }
    ...
    try {
      ...
      const resp = await fetch(
        `http://localhost:3000/api/metas/${metaId}/aportar`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
      ...
      if (!resp.ok) {
        throw new Error("Erro ao realizar aporte");
      }
      ...
      alert("Não foi possível registrar o aporte.");
    }
  };

  // Tirar dinheiro da meta -> PATCH /api/metas/:id/desaportar
  const handleTirarAporte = async (metaId) => {
    if (!valorAporte) {
      alert("Digite um valor para tirar da meta.");
      return;
    }
    ...
    try {
      ...
      const resp = await fetch(
        `http://localhost:3000/api/metas/${metaId}/desaportar`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
      ...
      if (!resp.ok) {
        throw new Error("Erro ao tirar aporte");
      }
      ...
      alert("Não foi possível tirar o aporte.");
    }
  };

  // Remover meta -> DELETE /api/metas/:id
  const handleRemoverMeta = async (id) => {
    ...
    try {
      ...
      if (!resp.ok || !dados.sucesso) {
        setErro(dados.erro || "Não foi possível remover a meta.");
        return;
      }
              ...
              <div className="mf-lista-container">
                {carregando ? (
                  ...
                  metas.map((meta) => {
                    ...
                    // trata dataMeta como "YYYY-MM-DD" e força horário local
                    const hoje = new Date();
                    ...
                    if (metaAlcancada) {
                      ...
                      statusLabel = "Atrasada, revise sua meta";
                          ...
                          <div className="mf-aporte-remover">
                            ...
                            <div className="mf-botoes-aporte">
                              ...
                              <button
                                ...
                                onClick={() => handleTirarAporte(meta.id)}
                              >
                                Tirar aporte
                              </button>
                              <button
                ...
                <form className="mf-form" onSubmit={handleAdicionarMeta}>
                  ...
                  <div className="form-group">
                    <label htmlFor="descricao">
                      Descrição (por que essa meta é importante?)
                    </label>
                    <input
                      type="text"
                      id="descricao"
